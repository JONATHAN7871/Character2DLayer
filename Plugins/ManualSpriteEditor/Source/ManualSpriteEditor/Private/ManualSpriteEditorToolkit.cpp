...
void FManualSpriteEditorToolkit::AutoTriangulateWithTransaction()
{
	...
	const FText NotificationText = FText::Format(
		...
		FText::AsNumber(ManualSprite->ManualGeometry.Triangles.Num()),
	...
	UE_LOG(LogTemp, Log, TEXT("Auto-triangulation completed with %d triangles"), ManualSprite->ManualGeometry.Triangles.Num());
}
...
void FManualSpriteEditorToolkit::AutoTriangulateWithMethodAndTransaction(ETriangulationMethod Method)
{
	...
	UE_LOG(LogTemp, Log, TEXT("Triangulation with method %d completed with %d triangles"), 
		   static_cast<int32>(Method), ManualSprite->ManualGeometry.Triangles.Num());
}
...
FManualSpriteEditorToolkit::FPolygonInfo FManualSpriteEditorToolkit::GetPolygonInfo() const
{
	...
	Info.VertexCount = Geometry.Vertices.Num();
	Info.TriangleCount = Geometry.Triangles.Num();
	Info.bIsConvex = ManualSprite->IsConvexPolygon();
	
	// ИСПРАВЛЕНИЕ: Вычисляем центроид локально вместо вызова ManualSprite->GetCentroid()
	...
	if (Info.VertexCount > 0)
	{
		Info.BoundingBoxMin = Geometry.Vertices[0].Position;
		Info.BoundingBoxMax = Geometry.Vertices[0].Position;
		...
		for (int32 i = 0; i < Info.VertexCount; i++)
		{
			...
			const FVector2D Current = Geometry.Vertices[i].Position;
			const FVector2D Next = Geometry.Vertices[NextIndex].Position;
...
{
	return ManualSprite && ManualSprite->ManualGeometry.Triangles.Num() > 0;
}

bool FManualSpriteEditorToolkit::CanSortVertices() const
{
	return ManualSprite && ManualSprite->ManualGeometry.Vertices.Num() >= 3;
}

bool FManualSpriteEditorToolkit::HasVertices() const
{
	return ManualSprite && ManualSprite->ManualGeometry.Vertices.Num() > 0;
}
